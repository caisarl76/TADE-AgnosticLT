...
class BNext_ride(nn.Module):
    def __init__(self, num_classes=1000, num_experts=1, use_norm=None, s=30, reduce_dimension=None):
        super(BNext_ride, self).__init__()
        drop_rate = 0.0
        ...
        self.feature = nn.ModuleList()
        # drop_rates = [x.item() for x in torch.linspace(0, drop_rate, (len(stage_out_channel)))]
        drop_rates = [0.0] * len(stage_out_channel)
        self.shared_backbone = self._make_layer(basicblock, stage_out_channel, drop_rates, 0, 7)
        self.experts = nn.ModuleList(
            [
                self._make_layer(basicblock, 
                                ...
                                drop_rates, 
                                7, 
                                len(stage_out_channel))
            for _ in range(num_experts)
            ]
        )
            
        if use_norm:
            self.linears = nn.ModuleList([NormedLinear(1024, num_classes) for _ in range(num_experts)])
        else:
    ...
    def _make_layer(self, block, channels, drop_rates, idx_start, idx_end):
        layers = []
        for i in range(idx_start, idx_end):
            if i == 0:
                layers.append(firstconv3x3(3, channels[i], 2))
            elif i == 1:
                layers.append((block(channels[i-1], channels[i], 1, drop_rate = drop_rates[i], mode = "bias")))
            elif channels[i-1] != channels[i] and channels[i] != channels[1]:
                layers.append(block(channels[i-1], channels[i], 2, drop_rate = drop_rates[i], mode = "scale" if i%2 == 0 else "bias"))
            else:
                layers.append(block(channels[i-1], channels[i], 1, drop_rate = drop_rates[i], mode = "scale" if i%2 == 0 else "bias"))
...
if __name__ == "__main__":
    ...
    model = BNext_ride(num_classes=100, num_experts=3).cpu()
    print(model)